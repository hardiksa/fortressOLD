...
import (
	...
	"github.com/kshlsa/fortress/v4/x/erc20/keeper"
...
func (suite KeeperTestSuite) TestRegisterCoin() {
	...
	testCases := []struct {
		...
		{
			"evm denom registration - afortress",
			func() {
				metadata.Base = "afortress"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
		...
		{
			"evm denom registration - wfortress",
			func() {
				metadata.Base = "wfortress"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
