...
var (
	now     = time.Now().UTC()
	acc1, _ = sdk.AccAddressFromBech32("fortress1qxx0fdsmruzuar2fay88lfw6sce6emamyu2s8h4d")
	acc2, _ = sdk.AccAddressFromBech32("fortress1nsrs4t7dngkdltehkm3p6n8dp22sz3mct9uhc8")
)
...
func (suite *GenesisTestSuite) TestClaimInitGenesis() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("afortress", sdk.NewInt(2_800)))
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("afortress", sdk.NewInt(400)))
				err := testutil.FundModuleAccount(suite.app.BankKeeper, suite.ctx, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			false,
